...
class ReLOL(ChunkBase):
    ...
    def __init__(self, data_num, feature_dim, chunk_size=1000, max_episode=1000, rollout_num=1000, hidden_dim=10,
                 metric='auc', learning_rate=0.0001, test_prop=0.33, pretrain_episode=100):
    ...
    def _update_chunk(self, data, label):
        ...
        # pretrain
        minority_ratio = sum(train_label == 1) / sum(train_label == 0)
        action_prob = np.zeros([1, train_num, 2])
        action_prob[0, train_label == 0] = [1 - minority_ratio, minority_ratio]
        action_prob[0, train_label == 1] = [minority_ratio, 1 - minority_ratio]

        for episode_i in range(self.pretrain_episode):
            print('pretrain episode: %d  ' % episode_i, end='')
            # rollout negative samples
            observation = np.expand_dims(np.c_[train_data, train_label], 0)
            pretrain_loss = self.pg.learn_pretrain_sampler(observation, action_prob)
            print('pretrain loss: %.4f  ' % pretrain_loss)
        ...
        for episode_i in range(self.max_episode):
            ...
            # rollout negative samples
            observation = np.expand_dims(np.c_[train_data, train_label], 0)
            sampler_action_prob, _ = self.pg.choose_sampler_action(observation)
            ...
            for rollout_i in range(self.rollout_num):
                ...
                self.pg.store_sampler_transition(observation, sampler_action, reward)
        ...
        sampler_action_prob, output_state = self.pg.choose_sampler_action(observation)
